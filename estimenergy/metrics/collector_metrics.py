...
class CollectorMetrics(Metrics):
    def __init__(self, collector: CollectorData):
        ...
        self.metrics = {
            metric: metric.create_gauge(registry=metric_registry) for metric in METRICS
        }
    ...
    async def update_metrics(self):
        ...
        for metric in METRICS:
            self.metrics[metric].labels(
                name=self.collector.name, id=self.collector.id
            ).set(data[metric.json_key])
