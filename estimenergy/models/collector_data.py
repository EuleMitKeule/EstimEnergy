...
class CollectorData(models.Model):
    ...
    async def get_metrics(self, date: datetime.date):
        return {
            metric.json_key: await self.get_metric(metric, date) for metric in METRICS
        }
    ...
    async def get_energy(self, metric: Metric, date: datetime.date):
        ...
        if metric.metric_period == MetricPeriod.MONTH:
            ...
            energy_total = sum(
                ...
                if metric.is_raw or energy_data.accuracy >= self.min_accuracy
            )
        ...
        if metric.metric_period == MetricPeriod.YEAR:
            ...
            for month_offset in range(12):
                ...
                accuracy = await self.get_accuracy(
                    Metric(MetricType.ACCURACY, MetricPeriod.MONTH, False, False), date
                )
                ...
                kwh_total += await self.get_energy(
                    Metric(MetricType.ENERGY, MetricPeriod.MONTH, True, metric.is_raw),
                    date,
                )
    ...
    async def get_cost(self, metric: Metric, date: datetime.date):
        ...
        if metric.metric_period == MetricPeriod.DAY:
            return kwh_cost + self.base_cost_per_month / get_days_in_month(
    ...
    async def get_accuracy(self, metric: Metric, date: datetime.date):
        ...
        accuracy_total = (
            sum(energy_data.accuracy for energy_data in energy_datas) / day_count
        )
    ...
    async def get_energy_datas(
        self, metric_period: MetricPeriod, date: datetime.date
    ) -> list[EnergyData]:
        if metric_period == MetricPeriod.DAY:
            return await EnergyData.filter(
                collector=self, year=date.year, month=date.month, day=date.day
            )

        if metric_period == MetricPeriod.MONTH:
            return await EnergyData.filter(
                collector=self, year=date.year, month=date.month
            )
